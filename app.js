...
async function registerFace() {
    try {
        ...
        if (data.faces && data.faces.length > 0) {
            ...
            alert('No hay un rostro. Colocate en un lugar optimo');
        }
    } catch (error) {
...
async function addFaceToFaceSet(faceToken) {
    ...
    const data = await response.json();
    console.log("Rostro agregado al Reconocimiento:", data);
}
...
async function loginFace() {
    try {
        if (!registeredFaceToken) {
            alert('Primero registra tu rostro.');
            return;
        }
        ...
        if (data.results && data.results.length > 0) {
            ...
            if (matchedFaceToken === registeredFaceToken) {
                ...
                alert("No se reconoce el rostro.");
            }
        } else {
